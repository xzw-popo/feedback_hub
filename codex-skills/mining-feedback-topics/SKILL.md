...
4. **Inspect.** Read the [backend contract](references/backend-contract.md). Copy client commands from the backend contract verbatim. Poll `get-run RUN_ID`; inspect funnel counts, data cutoff, vector watermark, and unresolved failures. For `paused_quota_exhausted`, repair quota/service access and run `resume RUN_ID` on the original run. For a repairable `failed` run other than source coverage/readiness, repair the reported service or configuration failure, then resume the original run. A source-coverage rejection occurs before a run is created; after the backend operator synchronizes the missing interval, repeat `create-run` so the backend binds the new source generation. Do not create a replacement run for a run that already exists in the same source generation.
