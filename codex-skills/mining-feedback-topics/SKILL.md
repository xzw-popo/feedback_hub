...
4. **Inspect.** Read the [backend contract](references/backend-contract.md). Copy client commands from the backend contract verbatim. Poll `get-run RUN_ID`; inspect funnel counts, data cutoff, vector watermark, and unresolved failures.
5. **Review.** Read the [review policy](references/review-policy.md). Fetch only `review-queue RUN_ID --output FILE`; when decisions are needed, submit `apply-overrides RUN_ID --file FILE` separately.
