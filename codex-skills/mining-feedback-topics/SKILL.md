...
2. **Validate.** Read the [topic spec](references/topic-spec.md). Populate each hard-scope field only with values explicit in the current request; leave every other hard-scope field empty except the time default. Set `mode: standard` unless the user explicitly requests all, complete, or exhaustive results; only then set `mode: exhaustive`. Six months alone does not mean completeness. Run [validate_topic_spec.py](scripts/validate_topic_spec.py) to atomically write a separate `PREPARED_SPEC_PATH`; repair errors.
