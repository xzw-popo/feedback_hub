...
Platform scope is canonicalized from the contract returned by capabilities. Common examples are `Windows -> Win` and `安卓 -> Android`; matching is case-insensitive and ignores whitespace. Prepared specs contain only `Win`, `Android`, `iOS`, or `Mac`. Keep the user's explicit platform intent, but do not guess ambiguous names such as PC, 电脑, 苹果, 手机, iPhone, or iPad. An unsupported or ambiguous platform blocks preparation and must be clarified; never remove it merely to obtain results.
