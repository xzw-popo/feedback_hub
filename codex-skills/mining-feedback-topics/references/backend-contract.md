...
Every run and exported artifact reports `mode`, `result_scope`, `matched_total`, `returned_feedback`, and `possibly_more_matches`. The only current `result_scope` values are `reviewed` and `representative`; copy the backend value and never invent `complete` or another value. In standard mode, classification is capped at 500 candidates and export at 100 confirmed rows. `result_scope=representative` or `possibly_more_matches=true` means the delivery is a representative result, not a complete list. Exhaustive mode removes the ordinary 100-row export cap but still reports any candidate safety-limit truncation; never hide that limitation.
