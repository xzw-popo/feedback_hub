...
def test_devcloud_archive_builds_with_bash_3_when_no_secret_env_exists(tmp_path):
    project = tmp_path / "project"
    project.mkdir()
    (project / "marker.txt").write_text("included", encoding="utf-8")
    git_marker = project / ".git" / "private-marker"
    git_marker.parent.mkdir()
    git_marker.write_text("excluded", encoding="utf-8")
    archive = tmp_path / "fixture.tar.gz"
    script = DEPLOY_SCRIPT.read_text(encoding="utf-8")
    archive_block = script.split("export COPYFILE_DISABLE=1\n", 1)[1].split(
        '\necho "[deploy] Uploading archive', 1,
    )[0]
    ...
    entries = set(subprocess.check_output(["tar", "-tzf", str(archive)], text=True).splitlines())
    assert "./marker.txt" in entries
    assert "./.git/private-marker" not in entries
